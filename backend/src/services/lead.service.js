...
// configured yet, the lead must still go through — this stays best-effort.
let conversation = null;
try {
    conversation = await getOrCreateConversation(lead);
} catch (error) {
    console.error("Conversation tracking unavailable:", error.message);
}

const message = `${buildOwnerLeadMessage(lead)}${conversation ? `
